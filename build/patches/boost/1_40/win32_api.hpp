...
inline void *create_file(const char *name, unsigned long access, unsigned long creation_flags, unsigned long attributes = 0)
{
    ...
    for (;;)
    {
        void * const res = CreateFileA(name, access, file_share_read | file_share_write | file_share_delete, 0, creation_flags, attributes, 0);
        if (tries > 100)
        {
            return res;
        }
        ...
        else if (GetLastError() != 32)  //not sharing_violation
        {
            return res;
        }
        Sleep(1);
