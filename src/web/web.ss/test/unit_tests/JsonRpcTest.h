...
inline void JsonRpcTest()
{
    ...
    {
        // id with boolean type — must throw InvalidRequest
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"id\": true}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // connectionName with wrong type (number instead of string)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"connectionName\": 123}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // typeId with wrong type (boolean instead of string or integer)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"typeId\": false}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // instanceId with wrong type (boolean instead of string or integer)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"instanceId\": true}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // entity with wrong type (string instead of object)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"setEntity\", \"params\": {\"entity\": \"notanobject\"}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // pending with wrong type (integer instead of boolean)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"pending\": 1}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // instanceIdPolicy with wrong type (integer instead of string)
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"instanceIdPolicy\": 42}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
    ...
    {
        // instanceIdPolicy with unrecognised string value
        try
        {
            auto json = "{\"jsonrpc\": \"2.0\", \"method\": \"open\", \"params\": {\"instanceIdPolicy\": \"InvalidPolicy\"}}";
            JsonRpcRequest r(json);
            r.Validate();
            CHECK(false);
        }
