...
bool ParseBoolParam(const std::unordered_map<std::string, std::string>& query,
...
void ParseUrlTarget(const std::string& target,
...
static const std::string kWrongVerb = "__wrong_verb__";
...
RestRoute RouteRestRequest(http::verb verb,
                           const std::vector<std::string>& seg,
                           const std::unordered_map<std::string, std::string>& /*query*/)
{
    const bool isGet    = (verb == http::verb::get);
    const bool isPut    = (verb == http::verb::put);
    const bool isPatch  = (verb == http::verb::patch);
    const bool isDelete = (verb == http::verb::delete_);
    const bool isPost   = (verb == http::verb::post);
    ...
    if (seg.size() >= 4 && seg[2] == "entities")
    {
        ...
        if (seg.size() == 5)
        {
            ...
            if (fifth == "isCreated")
            {
                // /connections/{id}/entities/{typeId}/isCreated — missing instanceId in path
                // instanceId must come from query param; handled in execution
                if (!isGet) return {kWrongVerb, {}, {}, {}};
