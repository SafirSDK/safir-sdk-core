...
struct Fixture
{
...

    {
        ...
        stopHandler.reset(new StopHandler(ioContext,
                                          communication,
                                          sp,
                                          doseMainCmdSender,
                                          controlConfig,
...
BOOST_AUTO_TEST_CASE( stop_external_node )
{
    ...
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 1234);});
...
BOOST_AUTO_TEST_CASE( stop_non_existing_external_node )
{
    stopHandler->Start();
    stopHandler->AddNode(1234, 1111);
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 5678);});
...
BOOST_AUTO_TEST_CASE( shutdown_external_node_resending )
{
    ...
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::SHUTDOWN, 5678);});
    ...
    BOOST_CHECK_EQUAL(nbrOfSend, 3);
    BOOST_CHECK_EQUAL(stopSafirNodeCb, 0);
    BOOST_CHECK(shutdownCb == 0);
    BOOST_CHECK(rebootCb == 0);
    BOOST_CHECK(stopSystemCb == 0);
}
...
BOOST_AUTO_TEST_CASE( stop_own_node )
{
    ...
    stopHandler->AddNode(1234, 1111);
    stopHandler->AddNode(5678, 2222);
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 9999);});
    ...
    BOOST_CHECK_EQUAL(nbrOfSend, 4); // 2 node types, 2 messages each
...
BOOST_AUTO_TEST_CASE( stop_own_node_while_pending_extrnal_node_stops  )
{
    ...
    stopHandler->Start();
    stopHandler->AddNode(1234, 1111);
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 1234);});  // stop external node
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 9999);});  // stop own node
...
BOOST_AUTO_TEST_CASE( stop_system )
{
    ...
    stopHandler->AddNode(1234, 1111);
    stopHandler->AddNode(5678, 2222);
    boost::asio::post(ioContext,[this](){controlCmdCallback(Control::STOP, 0);});
...
BOOST_AUTO_TEST_CASE( system_stop_cmd_while_in_system_stop_mode )
{
                                ...
                                bool /*deliveryGuarantee*/) -> bool
    {
        if (dataTypeIdentifier == stopOrderMsgTypeId)
        {
            ...
            if (nbrOfSentStopOrders == 1)
            {
                ...
                boost::asio::post(ioContext,[this_]
                               {
                                   auto stopOrder = Control::SerializeCmd(Control::STOP, 0);
    ...
    BOOST_CHECK_EQUAL(nbrOfSentStopOrders, 2);
