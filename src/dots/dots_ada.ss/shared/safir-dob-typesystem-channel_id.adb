...
package body Safir.Dob.Typesystem.Channel_Id is

   package Conv is new System.Address_To_Access_Conversions (Channel_Id_Type);

   procedure Throw (E : Ada.Exceptions.Exception_Id; Message : String := "")
                    renames  Ada.Exceptions.Raise_Exception;


   function Create_Channel_Id (Id : in Safir.Dob.Typesystem.Int_64) return Channel_Id_Type is
   begin
      return Channel_Id_Type'(Id           => Id,
                              Id_String    => Null_Unbounded_Wide_String,
                              Cached_Utf_8_String => Null_Unbounded_String);
   end Create_Channel_Id;


   function Create_Channel_Id (Id_Str : in Unbounded_Wide_String) return Channel_Id_Type is
      use Interfaces.C;
   ...
   begin
      if Length (Id_Str) = 0 then
         Throw (Software_Violation_Exception'Identity,
                "ChannelId can not be generated from null/empty string");
      end if;
      return Channel_Id_Type'(Id        => Internal (To_C (To_Utf_8 (Id_Str))),
                              Id_String => Id_Str,
                              Cached_Utf_8_String => Null_Unbounded_String);
   end Create_Channel_Id;
   ...
   function Create_Channel_Id (Id     : in Safir.Dob.Typesystem.Int_64;
                               Id_Str : in Unbounded_Wide_String) return Channel_Id_Type is
   begin
      return Channel_Id_Type'(Id        => Id,
                              Id_String => Id_Str,
                              Cached_Utf_8_String => Null_Unbounded_String);
   ...
   procedure Remove_String (Self   : in out Channel_Id_Type) is
   begin
      Self.Id_String := Null_Unbounded_Wide_String;
      Self.Cached_Utf_8_String := Null_Unbounded_String;
   end Remove_String;

   function "=" (L, R : Channel_Id_Type) return Boolean is
   begin
      return L.Id = R.Id;
   end "=";
   ...
   function To_String (Self : in Channel_Id_Type) return Unbounded_Wide_String is
   begin
      if Length (Self.Id_String) > 0 then
         return Self.Id_String;
      elsif Self.Id = Default_Channel.Id then
         return Default_Channel.Id_String;
      elsif Self.Id = All_Channels.Id then
         return All_Channels.Id_String;
      else
   ...
   function Get_Raw_Value (Self : in Channel_Id_Type) return Safir.Dob.Typesystem.Int_64 is
   begin
      return Self.Id;
   end Get_Raw_Value;


   function Get_Raw_String (Self : in Channel_Id_Type) return Unbounded_Wide_String is
   begin
      return Self.Id_String;
   end Get_Raw_String;
   ...
   function Utf_8_String (Self : in Channel_Id_Type) return Unbounded_String is
      Self_Ptr : constant Conv.Object_Pointer := Conv.To_Pointer (Self'Address);
   begin
