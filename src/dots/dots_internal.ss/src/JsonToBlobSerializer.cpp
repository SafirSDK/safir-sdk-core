...
******************************************************************************/
#ifdef _MSC_VER
#pragma warning(disable:4100) //unreferenced parameter in json_parser_read.hpp
#endif
...
namespace Internal
{
    ...
    void JsonToBlobSerializer::SerializeObjectContent(const std::string& typeName, std::vector<char>& blob, const boost::property_tree::ptree& members) const
    {
        ...
        if (!cd)
        {
            throw ParseError("JsonToBinary serialization error", "Json does not contain a known type. Typename: "+typeName, "", 201);
        }
        ...
        for (boost::property_tree::ptree::const_iterator memIt=members.begin(); memIt!=members.end(); ++memIt)
        {
            ...
            if (memIx<0)
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "", 203);
            }
            ...
            if (!md->IsArray())
            {
                ...
                catch (const boost::property_tree::ptree_error&)
                {
                    ...
                    throw ParseError("JsonToBinary serialization error", os.str(), "", 210);
                }
            }
            else
            {
                ...
                for (boost::property_tree::ptree::const_iterator arrIt=memIt->second.begin(); arrIt!=memIt->second.end(); ++arrIt)
                {
                    if (md->GetArraySize()<=arrayIndex)
                    {
                        ...
                        throw ParseError("JsonToBinary serialization error", os.str(), "", 216);
                    }
                    ...
                    catch (const boost::property_tree::ptree_error&)
                    {
                        ...
                        throw ParseError("JsonToBinary serialization error", os.str(), "", 211);
                    }
                }
            }
        }
    ...
    void JsonToBlobSerializer::SetMember(const MemberDescription* md,
                                         ...
                                         char* &beginningOfUnused) const
    {
        ...
        case EnumerationMemberType:
        {
            ...
            if (enumOrdinal<0)
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "", 214);
            }
        ...
        case EntityIdMemberType:
        {
            ...
            if (!typeIdString)
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "", 212);
            }
            if (!instanceIdString)
            {
                std::ostringstream os;
                os<<"EntityId member '"<<md->GetName()<<"' is missing the instanceId-element that specifies the instance.";
                throw ParseError("JsonToBinary serialization error", os.str(), "", 213);
            }
        ...
        case ObjectMemberType:
        {
            ...
            if (!xsiType)
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "", 202);
            }

            const ClassDescription* cd=m_repository->GetClass(DotsId_Generate64(xsiType->c_str()));
            if (!cd)
            {
                std::ostringstream os;
                os<<"Attribute 'type' on member "<<md->GetName()<<" does not specifying a known class. type="<<(*xsiType);
                throw ParseError("JsonToBinary serialization error", os.str(), "", 219);
            }
            else if (!IsOfType(m_repository, ObjectMemberType, cd->GetTypeId(), ObjectMemberType, md->GetTypeId()))
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "", 218);
            }
        ...
        case Watt32MemberType:
        {
            ...
            catch (const boost::bad_lexical_cast&)
            {
                ...
                throw ParseError("JsonToBinary serialization error", os.str(), "",  215);
            }
        }
            break;
    ...
    DotsC_TypeId JsonToBlobSerializer::StringToTypeId(const std::string& str) const
    {
        DotsC_TypeId tid=0;
        try
        {
    ...
    std::pair<DotsC_TypeId, const char*> JsonToBlobSerializer::StringToHash(const std::string& str) const
    {
    ...
    void JsonToBlobSerializer::CreateSpaceForDynamicMember(std::vector<char>& blob,
