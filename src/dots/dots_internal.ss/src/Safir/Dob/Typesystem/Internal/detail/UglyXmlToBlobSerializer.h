...
//            boost::optional<std::string> typeName=members.get_optional<std::string>("name");
//            if (!typeName)
//            {
//                std::ostringstream os;
//                os<<"The Xml does not contain member <name> that specifies the type. By the way, it seems like you are using the old-style and deprecated 'uglyXML'. Please don't!";
//                throw ParseError("UglyXmlToBinary serialization error", os.str(), "", 803);
//            }
...
//                }
//                m_blobLayout.template SetMember<bool>(boolVal, &blob[0], memIx, arrIx);
...
//            //ENSURE(usedSize<=blob.size(), <<"UglyXmlToBlobSerializer has written outside blob");
