...
namespace
{
    typedef std::map<std::string, boost::filesystem::path> FileLocations; //first is file name, second is full path (including file name)
    ...
    private:
                                  ...
                                  FileLocations& domFiles)
        {
            ...
            for (boost::filesystem::recursive_directory_iterator path(directory);
                 path != end; ++path)
            {
                ...
                {
                    const bool result = douFiles.insert(std::make_pair(filename,path->path())).second;
                ...
                {
                    const bool result = domFiles.insert(std::make_pair(filename,path->path())).second;
