...
EntityModel::EntityModel(const Safir::Dob::Typesystem::TypeId typeId,
...
{
    setupColumns();
    m_connection.SubscribeEntity(m_typeId, true, false, false, this);
}

EntityModel::~EntityModel()
{
    m_connection.UnsubscribeEntity(m_typeId, false, this);
}
...
QVariant EntityModel::headerData(const int section, const Qt::Orientation orientation, const int role) const
{
...
void EntityModel::OnNewEntity(const Safir::Dob::EntityProxy entityProxy)
{
    const auto result = m_entities.insert(std::make_pair(entityProxy.GetEntityId(), entityProxy.GetEntity()));
    const auto row = std::distance(m_entities.begin(), result.first);
    beginInsertRows(QModelIndex(), row, row);
    endInsertRows();
}

void EntityModel::OnUpdatedEntity(const Safir::Dob::EntityProxy entityProxy)
{
    const auto result = m_entities.insert_or_assign(entityProxy.GetEntityId(),
                                                    entityProxy.GetEntity());
    const auto row = std::distance(m_entities.begin(), result.first);
    emit dataChanged(index(row,0), index(row, m_columnInfoList.count() - 1));
}

void EntityModel::OnDeletedEntity(const Safir::Dob::EntityProxy entityProxy, const bool /*deprecated*/)
{
    const auto row = std::distance(m_entities.begin(), m_entities.find(entityProxy.GetEntityId()));
    beginRemoveRows(QModelIndex(),row,row);
    m_entities.erase(entityProxy.GetEntityId());
    endRemoveRows();
}

void EntityModel::setSecond64Format(const Second64Format format)
{
...
QVariant EntityModel::ContainerToVariant(const Safir::Dob::Typesystem::ContainerBase& container,
...
QStringList EntityModel::SequenceToStrings(const Safir::Dob::Typesystem::ContainerBase& container,
                                           const Safir::Dob::Typesystem::MemberType memberType,
                                           const Safir::Dob::Typesystem::TypeId memberTypeId) const
{
    using namespace Safir::Dob::Typesystem;
...
QStringList EntityModel::DictionaryToStrings(const Safir::Dob::Typesystem::DictionaryContainerBase& container,
...
QVariant EntityModel::Second64ToVariant(const Safir::Dob::Typesystem::Si64::Second seconds) const
{
