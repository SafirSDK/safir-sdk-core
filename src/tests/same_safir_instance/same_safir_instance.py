    ...
    with test_case("start_two_nodes_as_same_instance"),\
        ...
        with launch_node(args, safir_instance=1, node_id=2, first_node=False) as node2:
            ...
            if node2.SafirControlRunning():
                ...
                raise AssertionError("node2 is still alive but was expected to terminate")
            else:
                print("  node2 has terminated just as expected")
        ...
        # connect one more app and verify that node1 is still working
        print("  stop node1")
