...
###############################################################################
#
# Copyright Saab AB, 2023, 2026 (http://safirsdkcore.com)
#
...
async def check_pool(app, expected_registrations, expected_entities):
    ...
    for t, v in expected_registrations: log("    " + t + " - " + str(v))
    log("  Expected entities:")
    if len(expected_entities) < 500:
        for t, v in expected_entities: log("    " + t + " - " + str(v))
    else:
        log("Number of expected entities: " + str(len(expected_entities)))
...
async def lightnode_switch_to_new_system(args):
    with test_case("lightnode_switch_to_new_system"),\
         launch_node(args, safir_instance=9, node_id=9, start_dope=True) as system2:
        app2 = SafirApp(safir_instance=9, node_id=9)
        await app2.send('{"method": "registerEntityHandler", "params": {"typeId": "DoseTest.SynchronousPermanentEntity", "handlerId": "qwer", "injectionHandler":true}, "id": 103}')
        initialRegApp2 = [("Safir.Dob.NodeInfo:9", "Registered"),
                          ("DoseTest.SynchronousPermanentEntity:qwer", "Registered")]
        initialEntApp2 = [("Safir.Dob.NodeInfo:9", "_")]
        await check_pool(app2,initialRegApp2,initialEntApp2)
        await set_network_state(False, system2.session_id)
        #system 2 is now started, but with network off.

        with launch_node(args, safir_instance=1, node_id=1, start_dope=True) as system1,\
             launch_node(args, safir_instance=5, node_id=5) as lightnode:

            app1 = SafirApp(safir_instance=1, node_id=1)
            await app1.send('{"method": "registerEntityHandler", "params": {"typeId": "DoseTest.SynchronousPermanentEntity", "handlerId": "asdf", "injectionHandler":true}, "id": 103}')
            await app1.send('{"method": "registerEntityHandler", "params": {"typeId": "DoseTest.SynchronousPermanentEntity", "handlerId": "qwer", "injectionHandler":true}, "id": 103}')

            initialRegApp1 = [("Safir.Dob.NodeInfo:1", "Registered"),
                              ("Safir.Dob.NodeInfo:5", "Registered"),
                              ("DoseTest.SynchronousPermanentEntity:asdf", "Registered"),
                              ("DoseTest.SynchronousPermanentEntity:qwer", "Registered")]
            initialEntApp1 = [("Safir.Dob.NodeInfo:1", "_"),
                              ("Safir.Dob.NodeInfo:5", "_")]
            await check_pool(app1,initialRegApp1,initialEntApp1)
            #system 1 is now started, with a light node connected to it

            #turn off network to system 1 and on for system 2
            await set_network_state(False, system1.session_id)
            await set_network_state(True, system2.session_id)

            afterSwitchRegApp2 = [("Safir.Dob.NodeInfo:9", "Registered"),
                                  ("Safir.Dob.NodeInfo:5", "Registered"),
                                  ("DoseTest.SynchronousPermanentEntity:qwer", "Registered")]
            afterSwitchEntApp2 = [("Safir.Dob.NodeInfo:9", "_"),
                                  ("Safir.Dob.NodeInfo:5", "_")]
            await check_pool(app2,afterSwitchRegApp2,afterSwitchEntApp2)

# ===========================================
# main
# ===========================================
async def main(args):
    await lightnode_switch_to_new_system(args)
