...
#if defined _MSC_VER
  #pragma warning (push)
  #pragma warning (disable : 4127 4702 4267 4251 4275)
#endif
...
const CommandLineResults & HandleCommandLine(int argc, char* argv[])
{
    try
    {
        ...
        desc.add_options()
            ...
            ("fallback-handler", "Drops handler when an instance of Safir.Dob.PersistenceDataStatus is detected (default false)");
        ...
        if (vm.count("fallback-handler"))
        {
            results.isFallbackHandler = true;
        }
        else
        {
            results.isFallbackHandler = false;
        }
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        App app(commandLine.isFallbackHandler);
        app.Run();
    }
