...
namespace Internal
{
    ...
    public:
        ...
        IpcSubscriberImpl(boost::asio::io_context&  ioContext,
                          const std::string&        name,
                          const RecvDataCallback&   onRecvData)
            : m_strand(ioContext),
              m_stream(ioContext),
              m_connectRetryTimer(ioContext),
    ...
    public:
         ...
         * from the callback.
         *
         * @param ioContext [in] - io_context that will be used as engine.
         * @param name [in] - Ipc identification.
         * @param onRecvData [in] - Callback that will be called when a message is received.
         */
        IpcSubscriber(boost::asio::io_context&     ioContext,
                      const std::string&           name,
                      const RecvDataCallback&      onRecvData)
            : m_pimpl(std::make_shared<IpcSubscriberImpl<IpcSubscriberNoTest>>(ioContext, name, onRecvData))
        {
        }
