...
int main()
{
    ...
    for (;;)
    {
        try
        {
            ...
            std::wcout << "Probably got a subscription event, trying a read"<< std::endl;
            char * blob;
            ...
            DoseTest::GlobalEntityPtr gmp = boost::static_pointer_cast<DoseTest::GlobalEntity>(obj);
            std::wcout << gmp->Info().GetVal() << std::endl;
            Safir::Dob::Typesystem::Internal::Delete(blob);
        }
        catch(const Safir::Dob::NotFoundException &)
        {
            std::wcout << "Ooops, the entity did not exist!" << std::endl;
        }
    }
