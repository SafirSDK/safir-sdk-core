...
namespace Internal
{
        ...
        ,m_communication(distribution.GetCommunication())
    {
        ...
        distribution.SubscribeNodeEvents([=](const std::string& n, int64_t id, int64_t nt, const std::string& a){OnInjectNode(n, id, nt, a);},
                                         [=](int64_t id, int64_t nt){OnExcludeNode(id, nt);});
    ...
    void PoolHandler::OnInjectNode(const std::string& /*nodeName*/, int64_t nodeId, int64_t nodeTypeId, const std::string&/*dataAddress*/)
    {
        m_strand.dispatch([=]{m_nodes.insert({nodeId, nodeTypeId});});
    }

    void PoolHandler::OnExcludeNode(int64_t nodeId, int64_t /*nodeTypeId*/)
    {
        m_strand.dispatch([=]
        {
            if (m_nodes.erase(nodeId)>0)
            {
                if (m_poolDistributionRequests)
                {
                    m_poolDistributionRequests->ReceivedPoolDistributionCompleteFrom(nodeId);
                }
